...
export default function AffiliateCta({ affiliate, price }: { affiliate: Affiliate; price?: number }) {
  return (
    <span className="inline-flex flex-col items-end gap-1">
      ...
      {affiliate.termsHref && <a href={affiliate.termsHref} target="_blank" rel="nofollow noopener" className="font-data text-[9px] text-ink underline">{affiliate.termsLabel ?? "T&Cs apply"}</a>}
    </span>
  );
}
