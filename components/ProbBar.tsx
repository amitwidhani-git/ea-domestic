export default function ProbBar({ probs, pick }: { probs: { home: number; draw: number; away: number }; pick: "home" | "draw" | "away" }) {
  ...
  return (
    <div>
      <div className="flex h-2 w-full overflow-hidden rounded-full bg-panel2" role="img" aria-label={`H ${(probs.home*100).toFixed(0)}% D ${(probs.draw*100).toFixed(0)}% A ${(probs.away*100).toFixed(0)}%`}>
